...
#ifdef CC2520TCLICK
    /* Route FIFOP signal to IO1. */

    /* Collect FIFOP in exception channel B and output on GPIO1. */
    cc2520_reg_write(dev, CC2520_GPIOCTRL1, 0x22);

    /* select FIFOP exception from the 24-bit exception mask register. */
    cc2520_reg_write(dev, CC2520_EXCMASKB1, 0x10);
    /* Mask all other exceptions. */
    cc2520_reg_write(dev, CC2520_EXCMASKB0, 0x00);
    cc2520_reg_write(dev, CC2520_EXCMASKB2, 0x00);

    /* Clear FIFOP exception. */
    cc2520_clear_fifop_excep(dev);
#endif //CC2520TCLICK
...
bool cc2520_cca(cc2520_t *dev)
{
    ...
    return cc2520_strobe(dev,CC2520_INS_SSAMPLECCA);//gpio_read(dev->params.pin_cca); //TODO: CHANGE HERE
}
...
void cc2520_clear_fifop_excep(const cc2520_t *dev)
{
	...
	reg = cc2520_reg_read(dev, CC2520_EXCFLAG1);

	reg &= ~(1 << FIFOP_b);

	cc2520_reg_write(dev,CC2520_EXCFLAG1, reg);
