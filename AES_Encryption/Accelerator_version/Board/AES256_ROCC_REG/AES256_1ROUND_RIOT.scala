...
//========================================================///
  ...
  for (i <- c.KeyLength until c.KeyBSize) {
    ...
    key(i) := key(i-c.KeyLength) ^SubRotateWord(key(i-1)) ^ RconWord(((i/c.KeyLength)-1).asUInt)
      ...
      key(i) := key(i-c.KeyLength) ^ SubWord(key(i-1))
    }
    else {
...
//========================================================================================================///
class AES_Module(opcodes: OpcodeSet, c: AES_Config = new AES_256)(implicit p: Parameters) extends LazyRoCC(opcodes) {
 ...
 when(Cipher.io.start && Cipher.io.ready){
  ...
  when (io.cmd.fire()){
    when(ReceiveKey_0) {
     ...
     when(ReceiveKey_2) {
      key(4) := Cat(inputData1(7,0),inputData1(15,8),inputData1(23,16),inputData1(31,24))
      key(5) := Cat(inputData2(7,0),inputData2(15,8),inputData2(23,16),inputData2(31,24)) 
    }
     when(ReceiveKey_3) {
      key(6) := Cat(inputData1(7,0),inputData1(15,8),inputData1(23,16),inputData1(31,24))
      key(7) := Cat(inputData2(7,0),inputData2(15,8),inputData2(23,16),inputData2(31,24)) 
    }
    when(ReadKey_4) {
      data := key(inputData1)
    }
    when(ReceiveInput_6) {
      plaintext(index) := Cat(inputData1(7,0),inputData1(15,8),inputData1(23,16),inputData1(31,24))
      plaintext(index + 1.U) := Cat(inputData2(7,0),inputData2(15,8),inputData2(23,16),inputData2(31,24))
      index := index + 2.U
    }
    when(ReadInput_7) {
      data := Cat(plaintext(inputData1)(7,0),plaintext(inputData1)(15,8),plaintext(inputData1)(23,16),plaintext(inputData1)(31,24))
    }
    when(Set_en_8) {
      start := true.B
      en := true.B
