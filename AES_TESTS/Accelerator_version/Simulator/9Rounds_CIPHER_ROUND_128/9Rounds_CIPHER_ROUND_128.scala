...
class Cipher_Module(c: AES_Config)(implicit p: Parameters) extends Module{ //32bit decoder implementation without shifts. Compiler doesn't accept shifts of more than 20bits
  ...
  val in = RegInit(VecInit(Seq.fill(60)(0.U(32.W))))
  ...
  for (i <- c.KeyLength until c.KeyBSize) {
    ...
    //PARA JÀ DEI SKIP PARA KEY256 BITS (SUBS mod 4)
      ...
      in(i) := in(i-c.KeyLength) ^ SubWord(in(i-1))
    }
    else {
...
//========================================================///
  ...
  when(io.en){
      ...
      io.out(0) := Cat(En_Line0(temp0), En_Line1(temp0), En_Line2(temp0), En_Line3(temp0)) ^ io.key(0)
      io.out(1) := Cat(En_Line0(temp1), En_Line1(temp1), En_Line2(temp1), En_Line3(temp1)) ^ io.key(1)
      io.out(2) := Cat(En_Line0(temp2), En_Line1(temp2), En_Line2(temp2), En_Line3(temp2)) ^ io.key(2)
      io.out(3) := Cat(En_Line0(temp3), En_Line1(temp3), En_Line2(temp3), En_Line3(temp3)) ^ io.key(3)
    }.otherwise{
      temp0 := InvSubShiftRow(io.in(0), io.in(3), io.in(2), io.in(1)) ^ io.key(0)
      temp1 := InvSubShiftRow(io.in(1), io.in(0), io.in(3), io.in(2)) ^ io.key(1)
      temp2 := InvSubShiftRow(io.in(2), io.in(1), io.in(0), io.in(3)) ^ io.key(2)
      temp3 := InvSubShiftRow(io.in(3), io.in(2), io.in(1), io.in(0)) ^ io.key(3) 
      io.out(0) := Cat(De_Line0(temp0), De_Line1(temp0), De_Line2(temp0), De_Line3(temp0)) 
      io.out(1) := Cat(De_Line0(temp1), De_Line1(temp1), De_Line2(temp1), De_Line3(temp1)) 
      io.out(2) := Cat(De_Line0(temp2), De_Line1(temp2), De_Line2(temp2), De_Line3(temp2)) 
      io.out(3) := Cat(De_Line0(temp3), De_Line1(temp3), De_Line2(temp3), De_Line3(temp3)) 
    }
}
