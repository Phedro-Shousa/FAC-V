...
class KeyExpansion(c: AES_Config)(implicit p: Parameters) extends Module{ 
  ...
  val in = Input(Vec(KeyLength, UInt(32.W)))
...
class Cipher_Module(c: AES_Config)(implicit p: Parameters) extends Module{ //32bit decoder implementation without shifts. Compiler doesn't accept shifts of more than 20bits
  ...
  //=====================WIRES========================//
  val state0 = Wire(UInt(32.W))   
  val state1 = Wire(UInt(32.W))   
  val state2 = Wire(UInt(32.W))   
  val state3 = Wire(UInt(32.W))   
  ...
  //===================================================//
      ...
      state0 := message(0) ^ key(0)
      state1 := message(1) ^ key(1)
      state2 := message(2) ^ key(2)
      state3 := message(3) ^ key(3)
          
      round1.io.in(0) := state0
      round1.io.in(1) := state1
      round1.io.in(2) := state2
      round1.io.in(3) := state3
...
//========================================================///
  ...
  def Line0(w:UInt):UInt = x02(w(31,24)) ^ x03(w(23,16)) ^ w(15,8)      ^ w(7,0)
  def Line1(w:UInt):UInt = w(31,24)      ^ x02(w(23,16)) ^ x03(w(15,8)) ^ w(7,0)
  def Line2(w:UInt):UInt = w(31,24)      ^ w(23,16)      ^ x02(w(15,8)) ^ x03(w(7,0))
  def Line3(w:UInt):UInt = x03(w(31,24)) ^ w(23,16)      ^ w(15,8)      ^ x02(w(7,0))
  //======================================================================================================================//
    
      temp0 := SubShiftRow(io.in(0), io.in(1), io.in(2), io.in(3))
      temp1 := SubShiftRow(io.in(1), io.in(2), io.in(3), io.in(0))
      temp2 := SubShiftRow(io.in(2), io.in(3), io.in(0), io.in(1))
      temp3 := SubShiftRow(io.in(3), io.in(0), io.in(1), io.in(2))

      io.out(0) := Cat(Line0(temp0), Line1(temp0), Line2(temp0), Line3(temp0)) ^ io.key(0)
      io.out(1) := Cat(Line0(temp1), Line1(temp1), Line2(temp1), Line3(temp1)) ^ io.key(1)
      io.out(2) := Cat(Line0(temp2), Line1(temp2), Line2(temp2), Line3(temp2)) ^ io.key(2)
      io.out(3) := Cat(Line0(temp3), Line1(temp3), Line2(temp3), Line3(temp3)) ^ io.key(3)
}


class Inverse_Cipher_Module(c: AES_Config)(implicit p: Parameters) extends Module{ //32bit decoder implementation without shifts. Compiler doesn't accept shifts of more than 20bits
  ...
  val Sbox1 = Wire(Vec(256, UInt(8.W)))
  Sbox1 := VecInit(Sub.InvByteTable.map(_&0xff).map(_.U(8.W)))
  def SubShiftRow (r0:UInt, r1:UInt, r2:UInt, r3:UInt): UInt = Cat(Sbox1(r0(31,24)), Sbox1(r1(23,16)), Sbox1(r2(15,8)), Sbox1(r3(7,0)))
  //=====================REGISTERS========================//
  ...
  //=====================MODULES========================//
  val round1 = Module(new Inverse_ROUND_Module())  
  val round2 = Module(new Inverse_ROUND_Module())  
  val round3 = Module(new Inverse_ROUND_Module())  
  val round4 = Module(new Inverse_ROUND_Module())  
  val round5 = Module(new Inverse_ROUND_Module())  
  val round6 = Module(new Inverse_ROUND_Module())  
  val round7 = Module(new Inverse_ROUND_Module())  
  val round8 = Module(new Inverse_ROUND_Module())  
  val round9 = Module(new Inverse_ROUND_Module())  
  val round10 = Module(new Inverse_ROUND_Module())  
  val round11 = Module(new Inverse_ROUND_Module())  
  val round12 = Module(new Inverse_ROUND_Module())  
  val round13 = Module(new Inverse_ROUND_Module())  
  //===================================================//
      ...
      if(c.KeySize == 128){
      io.out(0) := SubShiftRow(round9.io.out(0), round9.io.out(3), round9.io.out(2), round9.io.out(1)) ^ key(40)
      io.out(1) := SubShiftRow(round9.io.out(1), round9.io.out(0), round9.io.out(3), round9.io.out(2)) ^ key(41)
      io.out(2) := SubShiftRow(round9.io.out(2), round9.io.out(1), round9.io.out(0), round9.io.out(3)) ^ key(42)
      io.out(3) := SubShiftRow(round9.io.out(3), round9.io.out(2), round9.io.out(1), round9.io.out(0)) ^ key(43)
      }
      if(c.KeySize == 192){
        io.out(0) := SubShiftRow(round11.io.out(0), round11.io.out(3), round11.io.out(2), round11.io.out(1)) ^ key(48)
        io.out(1) := SubShiftRow(round11.io.out(1), round11.io.out(0), round11.io.out(3), round11.io.out(2)) ^ key(49)
        io.out(2) := SubShiftRow(round11.io.out(2), round11.io.out(1), round11.io.out(0), round11.io.out(3)) ^ key(40)
        io.out(3) := SubShiftRow(round11.io.out(3), round11.io.out(2), round11.io.out(1), round11.io.out(0)) ^ key(51)
      }
      if(c.KeySize == 256){
        io.out(0) := SubShiftRow(round13.io.out(0), round13.io.out(3), round13.io.out(2), round13.io.out(1)) ^ key(56)
        io.out(1) := SubShiftRow(round13.io.out(1), round13.io.out(0), round13.io.out(3), round13.io.out(2)) ^ key(57)
        io.out(2) := SubShiftRow(round13.io.out(2), round13.io.out(1), round13.io.out(0), round13.io.out(3)) ^ key(58)
        io.out(3) := SubShiftRow(round13.io.out(3), round13.io.out(2), round13.io.out(1), round13.io.out(0)) ^ key(59)
      }
}

class Inverse_ROUND_Module()(implicit p: Parameters) extends Module{ //32bit decoder implementation without shifts. Compiler doesn't accept shifts of more than 20bits
  val io = IO(new Bundle {
  val in = Input(Vec(4, UInt(32.W)))
  val key = Input(Vec(4, UInt(32.W)))
  val out = Output(Vec(4, UInt(32.W)))
...
//==================================================///
  ...
  def Line0(w:UInt):UInt = x0e(w(31,24)) ^ x0b(w(23,16)) ^ x0d(w(15,8)) ^ x09(w(7,0))
  def Line1(w:UInt):UInt = x09(w(31,24)) ^ x0e(w(23,16)) ^ x0b(w(15,8)) ^ x0d(w(7,0))
  def Line2(w:UInt):UInt = x0d(w(31,24)) ^ x09(w(23,16)) ^ x0e(w(15,8)) ^ x0b(w(7,0))
  def Line3(w:UInt):UInt = x0b(w(31,24)) ^ x0d(w(23,16)) ^ x09(w(15,8)) ^ x0e(w(7,0))
  //======================================================================================================================//

      temp0 := SubShiftRow(io.in(0), io.in(3), io.in(2), io.in(1)) ^ io.key(0)
      temp1 := SubShiftRow(io.in(1), io.in(0), io.in(3), io.in(2)) ^ io.key(1)
      temp2 := SubShiftRow(io.in(2), io.in(1), io.in(0), io.in(3)) ^ io.key(2)
      temp3 := SubShiftRow(io.in(3), io.in(2), io.in(1), io.in(0)) ^ io.key(3)
 
      io.out(0) := Cat(Line0(temp0), Line1(temp0), Line2(temp0), Line3(temp0)) 
      io.out(1) := Cat(Line0(temp1), Line1(temp1), Line2(temp1), Line3(temp1)) 
      io.out(2) := Cat(Line0(temp2), Line1(temp2), Line2(temp2), Line3(temp2)) 
      io.out(3) := Cat(Line0(temp3), Line1(temp3), Line2(temp3), Line3(temp3)) 
}
...
class KeyExpansionExampleModuleImp(outer: KeyExpansionExample, c: AES_Config)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val ReceiveKey0 = funct === 0.U
  val ReceiveKey1 = funct === 1.U
  val ReceiveKey2 = funct === 2.U
  val ReceiveKey3 = funct === 3.U
  val ReadKey4 = funct === 4.U
  val ReadKey5 = funct === 5.U
  val ReceiveInput6 = funct === 6.U
  val ReceiveInput7 = funct === 7.U
  val ReadOut1_8 = funct === 8.U
  val ReadOut_9 = funct === 9.U
  val ReadOut2_10 = funct === 10.U
  val ReadOut3_11 = funct === 11.U
  val ReadOut4_12 = funct === 12.U
  val ReadOut5_13 = funct === 13.U
...
//=====================AES VALUES==================
  ...
  val Inverse_Cipher = Module(new Inverse_Cipher_Module(c))
  ...
  for (i <- 0 to c.NumberRounds) {
    for (j <- 0 until 4) {
      Inverse_Cipher.io.key((i*4)+j) := Keys.io.out(((c.NumberRounds - i)*4) + j)
    }
  }
  for (i <- 0 until 4) {
    Cipher.io.message(i) := plaintext(i)
    Inverse_Cipher.io.message(i) := plaintext(i)
  }
//=====================AES VALUES==================
  ...
  when (io.cmd.fire()){
    ...
    when(ReadOut_9) {
      data := Inverse_Cipher.io.out(inputData1)
    }
  }
