...
void encrypt(uint32_t* message_i, uint32_t* result, int size_i){
//=================SEND MESSAGE===============
  uint32_t dummy;
...
void decrypt(uint32_t* message_i, uint32_t* result, int size_i){
//=================SEND MESSAGE===============
  uint32_t dummy;
  for(int i = 0; i < size_i; i += 2){
    ROCC_INSTRUCTION(dummy, message_i[i], message_i[i+1], 6);
...
// ================================= 128===========================
  ...
  decrypt(result1.result_i, result1.result_i, size_result_i);
  printf("\nDECRIPTED\nInt:");
  for(int i = 0; i < size_result_i; i++){
    printf("%08x ", result1.result_i[i]);
  }
